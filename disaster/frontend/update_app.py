    ...
    try {
      if (authMode === "signup") {
        await axios.post(`${API_BASE}/auth/register`, {
          ...
          role: authRole,
          phone: authPhone,
          agency: "Disaster Volunteer Relief"
        });
        setAuthSuccess("Account created in Neon DB! Please enter password to Sign In.");
        setAuthMode("signin");
        setAuthPassword("");
      } else {
      ...
      setAuthError(err.response?.data?.detail || "Authentication error. Check credentials.");
    } finally {
        ...
        <div className="w-full max-w-md bg-slate-900/95 border border-slate-800 rounded-3xl p-8 shadow-2xl space-y-6 relative z-10">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-xs text-slate-400">
              {authMode === "signin" ? "Sign In to Access Operations & Your Records" : "Create Authorized Account in Neon PostgreSQL"}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 bg-slate-950 p-1 rounded-xl border border-slate-800 text-xs">
            <button
              ...
              className={`py-2 rounded-lg font-bold transition ${authMode === "signin" ? "bg-blue-600 text-white shadow" : "text-slate-400 hover:text-white"}`}
            >
              Sign In (Login)
            </button>
            <button
              ...
              className={`py-2 rounded-lg font-bold transition ${authMode === "signup" ? "bg-blue-600 text-white shadow" : "text-slate-400 hover:text-white"}`}
            >
              Sign Up (Register)
            </button>
          </div>
          ...
          <form onSubmit={handleAuthSubmit} className="space-y-3.5 text-xs">
            ...
            >
              ...
              <span>{authLoading ? "Processing..." : authMode === "signup" ? "Complete Registration" : "Sign In & Open Command"}</span>
            </button>
          </form>
          ...
          <div className="pt-2 border-t border-slate-800 text-center space-y-2">
            <button
              onClick={() => {
                ...
                localStorage.setItem("docp_session_user", JSON.stringify(demoCommander));
                localStorage.setItem("docp_session_token", "demo-token-commander-2026");
                setCurrentUser(demoCommander);
                setIsAuthenticated(true);
              }}
              className="text-xs text-blue-400 hover:text-blue-300 font-semibold underline underline-offset-4"
            >
              🚀 Demo Login as Commander (Supervisor Override)
            </button>
          </div>
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-slate-950 text-slate-100 font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* TAB 2: VOLUNTEER MANAGEMENT WITH OWNER PERMISSION */}
        ...
        <div className="fixed inset-0 z-[2000] bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg p-6 space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleSaveVolunteer} className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-400 mb-1">Deployment Status</label>
                ...
                <div>
                  ...
                  <input type="number" step="any" value={volForm.latitude} onChange={(e) => setVolForm({ ...volForm, latitude: parseFloat(e.target.value) })} className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2 text-slate-100" />
                </div>
              </div>
